#Problem 1
#Part A: Book Base Class (25 points)
...
class Textbook(Book):
    ...
    def highlight(self, page, text):
        if page < 1 or page > self.total_pages:
            print("Error: Page number must be between 1 and total pages.")
            return
...
#Problem 2:
#Part A: Workout base class 

class Workout:
    def __init__(self, name, duration_minutes, date):
        self.name = name
        self._duration_minutes = None
        self.duration_minutes = duration_minutes  # Use setter for validation
        self.date = date
...
#Part B: Specialized workout classes
...
class WeeklyLog:
    ...
    def summary(self):
        print(f"{self.week_label} Summary:")
        for workout in self._workouts:
            print(workout)
        print(f"Totals: {self.total_minutes} min, {self.total_calories:.1f} cal")
